...
Status:
  ...
  Phase 1F — IMPLEMENTED  : Forward-scatter JCAS detector (amp/phase LoS, MAD-CFAR)
                            = KLAIM DETEKSI utama. Tuning final: doppler_weight=0.1
                            (amp-dominant; Doppler di-demote krn tercemar CFO),
                            dopp_sigma floor=50Hz, k=4.5, min_score=2.5,
                            ma_len=90, cfar_len=120 (anti self-masking + responsif walk).
                            presence state machine hold=200 frame (frame-native; deteksi orang lewat, bukan event).
...
class ForwardScatterDetector:
    ...
    def __init__(self, fs_frame, ma_len=90, cfar_len=120, threshold_k=4.5,
                 min_score=2.5, amp_weight=1.0, doppler_weight=0.1,
                 presence_hold_frames=200):
        ...
        self._presence_timeout = int(max(1, presence_hold_frames)) # Presence state machine: OCCUPIED sampai sunyi presence_hold_frames frame (frame-native, TANPA konversi fs_frame)
        self._presence_counter = 0          # sisa frame sebelum balik EMPTY
    ...
    def update(self, avg_amp=None, H_est=None, cir_mag=None):
        ...
        dopp_sigma = max(self._mad_sigma(self.doppler_hist), 50.0)     # Hz
        score = float(np.sqrt(
        ...
        raw = bool(score > threshold and len(score_ref) >= max(5, self.ma_len // 2))
        if raw:
            self._presence_counter = self._presence_timeout   # reset timer tiap ada deteksi
        object_detected = self._presence_counter > 0          # OCCUPIED selama timer jalan
        if not raw and self._presence_counter > 0:
            self._presence_counter -= 1                        # hitung mundur saat sunyi
...
def run_hardware(fs, n_frames=100, tx_gain=80.0, rx_gain=70.0,
                 ...
                 max_range_m=12.0, direct_skip_bins=3, max_echoes=3,
                 calib_frames=400, save_clutter=None, load_clutter=None,
                 jcas=True, sense_ma_len=90, sense_cfar_len=120,
                 sense_threshold_k=4.5, sense_min_score=2.5, presence_hold_frames=200):
    """
    ...
    jcas_detector = ForwardScatterDetector(
        ...
        threshold_k=sense_threshold_k, min_score=sense_min_score, presence_hold_frames=presence_hold_frames
    ) if jcas else None
                ...
                for i in range(last_n, cur):
                    if phase1b and i < len(state["p1b_records"]):
                        ...
                        print(f"  {i+1:>4} | {rec['text']:>4} {rec['counter']:>3} {crc_str:>4} | "
                              ...
                              f"{rec.get('jcas_status','-'):>9} "
                              f"{'  warm' if rec.get('bg_phase')=='calib' else format(rec.get('jcas_score',0.0),'>6.2f')} | "
                              f"{state['ovf']:>4}{ph_tag}")
                    elif phase1b:
                        print(f"  {i+1:>4} | {'--':>4} {'--':>3} DROP | "
                              f"{'--':>7} {'--':>7} | {'--':>9} {'--':>6} | "
                              f"{state['ovf']:>4} [drop]")
                    else:
    ...
    if verbose:
        ...
        if phase1b and state["p1b_records"]:
            ...
            print(f"  FS = {fs/1e6:.2f} MS/s  | Packets: {n_decoded} (target {n_frames} frame)  "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fs", type=float, default=40e6,
                    help="Sample rate Hz (single run mode, default 40e6)")
    ap.add_argument("--fs-candidates", type=str,
                    default="5e6,10e6,15e6,20e6,30e6,35e6,40e6,45e6,50e6,60e6,70e6,80e6",
    ...
    ap.add_argument("--tx-gain", type=float, default=90.0)
    ap.add_argument("--rx-gain", type=float, default=76.0)
    ...
    ap.add_argument("--log-csv", type=str, default=None,
                    help="Per-frame ISAC log CSV. Auto-nama bila tak diisi: "
                         "isac_log_<ts>.csv (phase1b) atau run_<ts>.csv (non-phase1b).")
    ...
    # ── Phase 1C v1.3: frozen clutter map calibration ────────────
    ap.add_argument("--calib-frames", type=int, default=400,
                    help="Jumlah frame fase kalibrasi clutter map (default 400). "
    ...
    ap.add_argument("--sense-ma-len", type=int, default=90,
    ...
    ap.add_argument("--sense-min-score", type=float, default=2.5,
                    help="Lantai threshold deteksi JCAS (default 2.5; static <1.5, walk >5).")
    ap.add_argument("--sense-presence-frames", type=int, default=200,
                help="Presence hold (frame): tetap OCCUPIED sampai sunyi sekian FRAME (frame-native, tak bergantung FPS).")
    ...
    if args.fs_sweep:
        ...
        if log_csv is None:
            ...
            log_csv = f"isac_log_{ts}.csv" if args.phase1b else f"run_{ts}.csv"
        ...
        run_hardware(
            ...
            presence_hold_frames=args.sense_presence_frames,
        )
